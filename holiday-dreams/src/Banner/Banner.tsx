...
const Banner = (): JSX.Element => {
  ...
  return (
    <div
      className="flex flex-col justify-between banner h-[500px] p-2.5"
      style={{ backgroundImage: `url(` + bannerItem.url + `)` }}
    >
      <h2 className="text-5xl">{bannerItem.title}</h2>
      <a className="self-end" href={bannerItem.portfolioUrl}>
        Photo: {bannerItem.username} on Unsplash
      </a>
    </div>
  );
};
